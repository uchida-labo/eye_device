...
while True:
    ...
    if prev_frame is not None:
        ...
        #  輪郭抽出
        contours, hierarchy = cv2.findContours(rev, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE,offset=(xmin, ymin))
        # 面積でフィルタリング
        ...
        for x, y, width, height in rects:
            ...
            for i, cnt in enumerate(contours):
                ...
                #     pts = np.array(int(center[0]), int(center[1]))
                #     print(pts)                  #座標の表示(x[mm], y[mm])
                #     #円の中心点から右に5[pixel]、上に5[pixel]の場所に補正前の中心点の座標描画
                #     cv2.putText(bgr, '{}'.format(pts), (center[0]+5, center[1]+50), fontType, 1, (0, 255, 0),3)
