...
# カメラ設定
capture = cv2.VideoCapture(0+cv2.CAP_DSHOW)   #USBカメラ接続時は → 0+cv2.CAP_DSHOW
width = int(capture.set(cv2.CAP_PROP_FRAME_WIDTH, 1280))
height = int(capture.set(cv2.CAP_PROP_FRAME_HEIGHT, 720))
fps = capture.set(cv2.CAP_PROP_FPS, 100)
